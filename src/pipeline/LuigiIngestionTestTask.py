...
class IngestionTest(marbles.core.TestCase, marbles.mixins.DateTimeMixins):

  def __int__(self, date, path_cred, data):
    super().__init__()
    self.date = date
...
class IngestionTest(CopyToTable):
  ...
  def requires(self):
	...
	def rows(self):
		...
		test = ingestion_metadata(date = self.date, path_cred = self.path_cred, data = self.input())
    ...
    data_test = {
      ...
      "processing_date": [date.today(), date.today(), date.today()],
