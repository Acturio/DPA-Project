...
from src.pipeline.LuigiTrainingTask import TrainingModelTask
...
class TrainingModelTest(marbles.core.TestCase, marbles.mixins.DateTimeMixins):

  def __init__(self, my_date, path_cred, data):
    super(TrainingModelTest, self).__init__()
    self.date = my_date
  ...
  def test_n_models_validation(self):
    data = self.data
    nmodels = len(data)
    self.assertGreater(nmodels, 0, note = "El archivo debe contener al menos 1 modelo")
    return True


class TrainingModelTestTask(CopyToTable):
